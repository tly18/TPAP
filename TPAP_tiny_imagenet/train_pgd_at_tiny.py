    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                # print(x_adv.shape,model(x_adv).shape, y.shape)
...
if __name__ == '__main__':
    ...
    valdir = os.path.join('./tiny-imagenet-200/val')
    ...
    # model = WideResNet(depth=34, num_classes=200, widen_factor=10, dropRate=0.0).to(device)    
    ...
    pth_save_dir = os.path.join('./para/pgd_AT_resnet18_tiny_imagenet/')
    ...
    for _epoch in range(epoch):   
        ...
        for i, (data, target) in enumerate(train_loader):
            ...
            data_adv_one = generation_adv(model,data,target,step_size=0.007,epsilon=0.031,perturb_steps=10,distance='l_inf')
            model.train()
            optimizer.zero_grad()  # 梯度归零
            ...
            with torch.no_grad():
                ...
                loss_nat = criterion(output_nat, target)  # 交叉熵损失v
                #nat
        ...
        for i, (data, target) in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                test_nat_correct += torch.sum(prediction==target)
            ...
            prediction_adv = output_adv.max(1)[1]
            test_adv_correct += torch.sum(prediction_adv==target)
        ...
        # str(round(((train_adv_two_correct / train_total)*100).item(),4)) + 
        # str(round(train_adv_two_loss,4))+ 
